    ...
    if (editor) {
      ...
      for (server of this.servers) {
        if (server.editor == editor) {
          atom.notifications.addError(`A server already exists for the file "${server.editor.getTitle()}". Your ID is: ${server.conn.id}`, {dismissable: false});
          return false;
        }
      }
      ...
      buttons: [
        {
          ...
          onDidClick: () => {
            ...
            let name      = document.querySelector('#pair-programming-z_client-name').value;
            name = name || 'Anonymous';

            message.dismiss();

            atom.workspace.open().then((editor) => {
              client = new EditorClient(editor, serverId, name);
