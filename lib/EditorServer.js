    ...
    this.conn.onDidData((payload) => {
      ...
      switch(payload.event) {
        ...
        case 'change':
          ...
          // const marker = this.editor.markBufferRange(payload.data.mark, { invalidate: 'never' });
          // this.editor.decorateMarker(marker, { type: 'highlight', class: `${this.users[payload.signature].color} pair-programming-z_overlay` });
        break;
        case 'cursor':
          for (let signature in this.users) {
    ...
    buffer.onDidChange((event) => {
      if (waitForTriggerChange < 1) {
        this.conn.send({
          event: 'change',
          data: {
            delete: event.oldRange.serialize(),
            insert: {
              at: event.newRange.start.serialize(),
              text: event.newText,
            },
            mark: event.newRange.serialize(),
          },
        });
      } else {
